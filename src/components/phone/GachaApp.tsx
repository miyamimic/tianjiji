...
export default function GachaApp({
  ...
  const executePullFlow = useCallback(async (pullCount: number) => {
    ...
    setCurrentBatchPulls(pullResult.items);
    ...
    // CALL ③ (Gacha Result Profile) during summon animation (free thinking time)
    ...
    // Simulate Summon Animation Duration (with Agent Skip Click)
    const animDurationMs = 3800; // 3.8s initial cinematic animation
    await new Promise((r) => setTimeout(r, animDurationMs));

    // Agent executes skip if still in summon screen
    let profileResult: any;
    try {
      profileResult = await Promise.race([
        resultProfilePromise,
        new Promise((r) => setTimeout(r, 1200)), // short timeout fallback
      ]);
    } catch (e) {
      // fallback
    }
    ...
    if (!profileResult) {
      profileResult = {
        ...
        evaluations: [],
        summary_bubble: '十连共鸣完成，让我们一张张揭开卡面！',
      };
    }

    // Move cursor to skip position and click to skip to results
    ...
    moveCursorTo(skipX, skipY, 0.4);
    await new Promise((r) => setTimeout(r, 450));
    await triggerCursorClick();

    // Transition to Card Flipping Screen (v2 core)
    setCurrentScreen('result_flipping');
    setIsAgentThinking(false);

    // Execute v2 Card Flipping Driver via Agent Virtual Cursor
  ...
  const runAgentCardFlippingSequence = async (
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      // f. Show evaluation bubble
      const matchingEval = evaluations.find((e: any) => e.card_index === i);
      let evalText = matchingEval?.text;
      if (!evalText) {
        if (currentCardItem.card.char_evaluation_pool && currentCardItem.card.char_evaluation_pool.length > 0) {
          const pool = currentCardItem.card.char_evaluation_pool;
          evalText = pool[Math.floor(Math.random() * pool.length)];
        } else if (currentCardItem.card.rarity === 'SSR') {
          evalText = `金光！是SSR【${currentCardItem.card.name}】！太棒啦～`;
        } else if (currentCardItem.card.rarity === 'SR') {
          evalText = `紫光SR【${currentCardItem.card.name}】到手！`;
        }
      }
  ...
  const handleSendUserChatMessage = async (customText?: string) => {
    ...
    // Call Endpoint ④ (Async user message response)
    const progressSummary = `已抽 ${totalPulls} 发，已出SSR: ${ssrObtainedList.join('、') || '暂无'}，当前井进度 ${sparkCount}/${poolConfig.spark_count}`;
    const llmConfig = loadLlmConfig();

    try {
      const resp = await generateGachaUserResponse(llmConfig, fallbackCharacter, progressSummary, text);
      if (resp.response_bubble) {
        showBubble(resp.response_bubble, 'bubble_to_user', 4500);
      }

      if (resp.action === 'stop') {
        setTimeout(() => handleEndGachaSession(), 1500);
      } else if (resp.action === 'change_target') {
        setUserInstruction(`主控调整目标：${text}`);
      }
    } catch (err) {
  ...
  const handleEndGachaSession = async () => {
    ...
    try {
      const ending = await generateGachaEnding(llmConfig, fallbackCharacter, {
        totalPulls,
        userInstruction,
        goalAchieved: ssrObtainedList.length > 0 || totalPulls >= 80,
        ssrList: ssrObtainedList,
  ...
  return (
    <div className="w-full h-full flex flex-col bg-stone-950 text-white relative select-none overflow-hidden font-sans">
      ...
      {/* 2. MAIN GACHA CONTAINER & CANVAS */}
      <div className="flex-1 flex flex-col items-center justify-center p-1 sm:p-2 relative overflow-hidden bg-black/90">
        
        <div
          ref={containerRef}
          className="w-full max-w-[450px] h-[520px] sm:h-[580px] max-h-[72vh] relative overflow-hidden rounded-2xl border border-stone-800 bg-black shadow-2xl"
