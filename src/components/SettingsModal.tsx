  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 animate-in fade-in-0 duration-200">
      <div
        ...
        onClick={onClose}
      />
      <div className="relative z-10 w-full max-w-2xl rounded-2xl border border-white/10 bg-[hsl(220_22%_13%)] shadow-2xl max-h-[88vh] flex flex-col overflow-hidden">
        {/* Header */}
        <div className="flex items-center justify-between border-b border-white/10 px-5 py-3.5 bg-[hsl(220_22%_13%)] select-none">
          <div>
            ...
            <p className="text-[10px] text-white/40 mt-0.5">
              LLM 结构化提示词分层注入、六维情感自然平复衰减与角色卡深度编辑
            </p>
          </div>
          <button
            onClick={onClose}
        ...
        {/* Tab Navigation */}
        <div className="flex border-b border-white/5 bg-black/20 text-xs font-medium px-4 select-none shrink-0">
          <button
        ...
        {/* Content */}
        <div className="flex-1 p-5 overflow-y-auto min-h-0 bg-black/5 custom-scrollbar">
          {activeTab === 'prompt' && (
