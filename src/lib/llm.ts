...
现在你要帮主控抽卡。
当前卡池：${poolConfig.pool_name}
可用按钮：${buttonsList}
主控说："${userInstruction || '帮我抽几发看看手气！'}"

你的第一步要做什么？移动虚拟光标到哪个按钮（或空白处）？
...
\`\`\`json
{
  ...
  "bubble_to_user": "你确定要我抽？我今天手气可说不准哦～"
}
\`\`\``;
...
export async function generateGachaUserResponse(
  ...
  const prompt = `你是角色「${character.name}」，口吻风格：${character.core.speech_filter}。
你正在帮主控抽卡。

当前抽卡进度状态：${progress}
主控说："{${userMessage}}"

请以你的角色性格回复主控，并决定是否改变下一步行动：
- continue：继续原计划抽卡/翻卡
- stop：听从主控暂停/收手
- change_target：改变目标（如从十连改单抽，或去查看概率/详情）
- go_to：去某个页面（如看抽卡记录）

输出纯合法JSON：
\`\`\`json
{
  "response_bubble": "收到！既然你发话了，那我们再来最后一发冲刺！",
  "action": "continue"
}
\`\`\``;
  ...
  try {
    const raw = await callLlm(config, [
      { role: 'system', content: `你是「${character.name}」，严格遵守抽卡互动协议，输出纯净JSON。` },
      { role: 'user', content: prompt },
    ]);
    const match = raw.match(/\{[\s\S]*\}/);
    if (match) {
