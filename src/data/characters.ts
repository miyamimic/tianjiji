...
export const MOCK_CHARACTERS: Character[] = [LU_CHEN, A_YE];
...
export function getCharacterById(id: string): Character | undefined {
  ...
  return list.find((c) => c.character_id === id);
}

export function getCharacterOrDefault(id?: string): Character {
  const list = getSavedCharacters();
  if (id) {
    const c = list.find((x) => x.character_id === id);
    if (c) return c;
  }
  return list[0];
}
